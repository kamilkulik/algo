...
@pytest.mark.parametrize('input, target, result', test_cases, ids = ids)
def test_move_element_to_end(input, target, result):
    assert move_element_to_end(input, target) == result